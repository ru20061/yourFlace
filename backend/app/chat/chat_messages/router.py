...
async def get_chat_messages_list(
    ...
    """목록 조회"""
    filters = {"chat_room_id": room_id} if room_id is not None else None
    items = await crud.chat_message_crud.get_multi(db, skip=skip, limit=limit, filters=filters)
    total = await crud.chat_message_crud.count(db, filters=filters)

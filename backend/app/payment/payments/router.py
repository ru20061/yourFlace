...
from app.payment.payments import crud, schemas
...
async def create_payments(
    ...
    """생성"""
    obj = await crud.payment_crud.create(db, obj_in)
    return obj
...
async def get_payments(
    ...
    """단건 조회"""
    obj = await crud.payment_crud.get(db, id)
    if not obj:
        raise HTTPException(
...
async def get_payments_list(
    ...
    """목록 조회"""
    items = await crud.payment_crud.get_multi(db, skip=skip, limit=limit)
    total = await crud.payment_crud.count(db)
    
    return schemas.PaymentList(
        items=items,
        total=total,
        skip=skip,
        limit=limit
...
async def update_payments(
    ...
    """업데이트"""
    obj = await crud.payment_crud.update(db, id, obj_in)
    if not obj:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Payment not found"
...
async def delete_payments(
    ...
    """삭제 (status='D')"""
    success = await crud.payment_crud.delete(db, id)
    if not success:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Payment not found"

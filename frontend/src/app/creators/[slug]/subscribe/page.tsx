...
export default function SubscribePage() {
    ...
    (async () => {
      try {
        ...
        const alreadySubscribed = subsRes.items.some(
          (s) => s.creator_id === creatorRes.id && s.fan_id === user?.id && s.status === "subscribed"
        );
        ...
        const creatorPlans = plansRes.items.filter(
          (p) => p.creator_id === creatorRes.id && p.is_active
        );
        setPlans(creatorPlans);
        if (creatorPlans.length > 0) {
          setSelectedPlan(creatorPlans[0]);
        }
      } catch {
        setCreator(null);
      } finally {
  ...
  const handlePayment = async () => {
    if (!user || !creator || !selectedPlan) return;
    setProcessing(true);
    setError("");
    ...
    try {
      ...
      // 3. 구독 완료 → 크리에이터 페이지로 이동 (새로고침으로 사이드바 갱신)
    ...
    return (
      <div className="subscribe-page">
        <div className="subscribe-header">
          <Link href={`/creators/${slug}`} className="subscribe-back-btn">
      ...
      <div className="subscribe-artist-info">
        <div className="subscribe-artist-avatar">
          {creator.profile_image ? (
            ...
            <span>{creator.stage_name[0]}</span>
          )}
        </div>
        <div className="subscribe-artist-name">{creator.stage_name}</div>
      </div>

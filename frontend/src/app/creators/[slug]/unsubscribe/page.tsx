...
export default function UnsubscribePage() {
  ...
  const handleCancelAndRefund = async () => {
    if (!user || !creator || !subscription) return;
    if (!reasonCode) {
    ...
    try {
      ...
      // 5. 완료 → 크리에이터 페이지로 이동 (새로고침으로 사이드바 갱신)
  ...
  return (
    <div className="unsubscribe-page">
      ...
      <div className="unsubscribe-header">
        <Link href={`/creators/${slug}`} className="unsubscribe-back-btn">
      ...
      <div className="unsubscribe-artist-info">
        ...
        <div className="unsubscribe-artist-name">{creator.stage_name}</div>
      ...
      <div className="unsubscribe-notice">
        <p>구독을 취소하면 해당 크리에이터의 콘텐츠를 더 이상 이용할 수 없습니다.</p>
      ...
      <div className="unsubscribe-actions">
        <Link href={`/creators/${slug}`} className="unsubscribe-cancel-btn">
          돌아가기
        </Link>
        <button

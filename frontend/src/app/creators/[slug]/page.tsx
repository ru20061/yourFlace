...
import "./creator-detail.css";
import "../../search/search.css";

const TABS = ["유저 포스트", "크리에이터 포스트", "이미지", "동영상", "이벤트", "검색"] as const;

const SEARCH_CATEGORIES = ["전체", "유저 포스트", "크리에이터 포스트", "이미지", "동영상", "이벤트"] as const;
...
export default function ArtistDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // slug로 크리에이터 조회
        const creatorRes = await api.get<Creator>(`/creators/by-slug/${encodeURIComponent(slug)}`);
        setCreator(creatorRes);
        const creatorId = creatorRes.id;
        ...
        const [postsRes, imagesRes, videosRes, linksRes, subsRes, eventsRes, tagsRes] =
          await Promise.all([
            ...
            api.get<PaginatedResponse<CreatorImage>>(`/creator-images/?skip=0&limit=100`),
            api.get<PaginatedResponse<CreatorVideo>>(`/creator-videos/?skip=0&limit=100`),
            api.get<PaginatedResponse<CreatorSocialLink>>(`/creator-social-links/?skip=0&limit=100`),
        ...
        // 크리에이터 포스트 (백엔드에서 author_name 포함)
        const creatorPosts = postsRes.items
          .filter((p) => p.author_id === creatorId && p.is_artist_post)
        ...
        setImages(
          imagesRes.items
            .filter((i) => i.creator_id === creatorId)
            .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
        );
        ...
        setSocialLinks(
          linksRes.items
            .filter((l) => l.creator_id === creatorId && l.is_active)
            .sort((a, b) => a.priority - b.priority)
        );

        // 구독자 수 + 내 구독 여부
        const creatorSubs = subsRes.items.filter(
          (s) => s.creator_id === creatorId && s.status === "subscribed"
        );
        setSubscriberCount(creatorSubs.length);

        if (user) {
          const mySub = creatorSubs.find((s) => s.fan_id === user.id);
  ...
  const handleWritePost = async () => {
    if (!user || !creator || !writeContent.trim()) return;
  ...
  const calendarDateItems = useMemo(() => {
    ...
    for (const p of posts) {
      add(toDate(p.published_date || p.created_at), { type: "artistPost", label: "크리에이터 포스트" });
    }
  ...
  const imageDate = (i: CreatorImage) => toDateStr(i.published_date || i.created_at);
  const videoDate = (v: CreatorVideo) => toDateStr(v.published_date || v.created_at);
  ...
  const totalSearchResults = useMemo(() => {
    ...
    if (cat === "전체" || cat === "크리에이터 포스트") count += searchResults.artistPosts.length;
  ...
  if (!creator) {
    return (
      <div className="artist-detail-page">
        <div className="feed-empty">크리에이터를 찾을 수 없습니다</div>
      </div>
    );
  }
    ...
    <div className="artist-detail-page">
      ...
      <div className="artist-profile-header">
        ...
        <div className="artist-profile-info">
          <div className="artist-avatar-large">
            <span className="artist-avatar-text">{creator.stage_name[0]}</span>
          </div>
          <h1 className="artist-name">{creator.stage_name}</h1>
          <p className="artist-bio">{creator.notes ?? "소개가 아직 없습니다."}</p>
      ...
      {!isSubscribed ? (
        <div className="artist-content-locked">
          ...
          <p className="artist-locked-desc">구독 후 {creator.stage_name}의 모든 콘텐츠를 확인할 수 있습니다</p>
        </div>
      ) : (
      /* 콘텐츠 피드 */
        ...
        {activeTab === "크리에이터 포스트" && (
          <div className="artist-feed-list">
            {posts.length > 0 ? (
              ...
              <div className="feed-empty">크리에이터 포스트가 아직 없습니다</div>
            )}
          </div>
        )}
        ...
        {activeTab === "검색" && (
          <div className="search-page">
            ...
            <div className="search-bar">
              ...
              <input
                type="text"
                placeholder={`${creator.stage_name}의 콘텐츠 검색...`}
                value={searchQuery}
            ...
            <div className="search-results">
              ...
              {/* 아티스트 포스트 결과 */}
              {(searchFilters.category === "전체" || searchFilters.category === "크리에이터 포스트") &&
                searchResults.artistPosts.length > 0 && (
                  <div className="search-result-section">
                    <div className="search-result-section-title">크리에이터 포스트</div>
